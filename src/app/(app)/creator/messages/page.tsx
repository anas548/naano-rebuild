...
import { MessageComposer } from "@/components/creator/message-composer";
...
export default async function CreatorMessagesPage({
  ...
  const selectedId =
    typeof threadParam === "string" ? threadParam : conversations[0]?.id;
        ...
        <aside className="flex min-h-0 flex-col border-r border-[#e6e8ef]">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto pb-4">
            {conversations.map((conversation) => {
              const name = counterpartName(conversation, true);
        ...
        <section className="flex min-h-0 flex-col">
          <header className="border-b border-[#e6e8ef] px-6 py-4">
            <p className="text-[0.9375rem] font-semibold text-ink">
              {thread ? counterpartName(thread as never, true) : "Messages"}
            </p>
            <p className="text-[0.8125rem] text-ink/50">
              {thread?.collaboration
                ? thread.collaboration.campaign.name
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {message.sender?.firstName ?? "NaanoBot"} ·{" "}
