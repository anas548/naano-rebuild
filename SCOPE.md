...
| **LinkedIn identity enrichment** | The public profile URL is fetched (`src/lib/linkedin-scrape.ts`) and its `og:title`/`og:image` meta tags — the same ones a link-preview unfurl reads, no login required — pick up a real name and photo, overwriting what was typed at signup. Shows up everywhere a creator's avatar appears: onboarding preview, topbar, My card, the public card page, the brand Marketplace card. Any failure (private profile, timeout, blocked, markup change) fails silently and never blocks onboarding — the typed name and an initials avatar are always the fallback |
...
| **AI Matching (Creators tab)** | Real, via DeepSeek (`src/lib/ai-matching.ts`, model `deepseek-flash`): the brand's ICPs, value proposition and active campaign brief, plus the real Marketplace pool (name, industries, headline, price, country — up to 60 candidates), go to a JSON-mode call that returns up to 4 picks with a one-sentence reason each, grounded only in the data actually provided. DeepSeek's JSON mode isn't schema-enforced server-side (unlike OpenAI's), so the shape is spelled out in the prompt and every returned id is re-checked against the real candidate list before being trusted — a hallucinated id can't slip through. Results render as the same real `MarketplaceCreatorCard` used on the Marketplace tab, reason included, with a fully working Add button. Any failure — no key, no credit, malformed output — shows a clear error, never a fabricated result |
| **NaanoBot assistant bar** | Real, via the same DeepSeek call (`src/lib/assistant.ts`, `src/lib/llm.ts`), mounted once in the root layout so it's on every page. Single-turn — no conversation history kept. On the creator's Opportunities page, every question is treated as "which opportunity fits me best": the model ranks the real open campaigns against the creator's real profile (never stating a fabricated acceptance percentage — there's no such data), and the page highlights/reorders the matching cards via a shared `AssistantContext`. Everywhere else, on both sides, it's a basic FAQ chatbot about how Naano works |
...
| **LinkedIn import** | The URL is validated, stored, and its public og:title/og:image are used for identity (see above), but nothing beyond that — no posts, no follower counts, no engagement — is fetched. The "import is temporarily paused" notice from the onboarding steps has been removed by request, since it read as contradicting the real name/photo shown right next to it | Agreed to hardcode the *data* import as paused. Consequence: follower counts and post metrics stay at zero even though the name/photo now come from the real profile |
| **Onboarding step 3 loading beat** | The "AI matching…" spinner during signup is still a static ~2s beat, then opens the dashboard | Explicit decision, unrelated to the Creators tab feature below — no model call at signup time |
