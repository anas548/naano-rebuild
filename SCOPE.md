...
| **AI Matching (Creators tab)** | Real, via OpenAI (`src/lib/ai-matching.ts`): the brand's ICPs, value proposition and active campaign brief, plus the real Marketplace pool (name, industries, headline, price, country — up to 60 candidates), go to a `gpt-4o-mini` structured-output call that returns up to 4 picks with a one-sentence reason each, grounded only in the data actually provided (the model is instructed never to invent facts, and only ids present in the candidate list are accepted back). Results render as the same real `MarketplaceCreatorCard` used on the Marketplace tab, reason included, with a fully working Add button. Any failure — no key, no quota, malformed output — shows a clear error, never a fabricated result |
