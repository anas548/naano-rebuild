...
| **LinkedIn import** | The URL is validated, stored, and its public og:title/og:image are used for identity (see above), but nothing beyond that — no posts, no follower counts, no engagement — is fetched; the "temporarily paused" notice still shows | Agreed to hardcode the *data* import as paused. Consequence: follower counts and post metrics stay at zero even though the name/photo now come from the real profile |
